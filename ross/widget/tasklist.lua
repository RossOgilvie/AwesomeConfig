...
function new(label, buttons)
    ...
    local widgets = { }
    widgets.imagebox = { }
    widgets.textbox  = { margin    = { left  = 15,
                                       right = 20 },
                         bg_resize = true,
                         bg_align  = "left"
                       }
...
local function widget_tasklist_label_common(c, args)
    ...
    local fg_secondary = args.fg_secondary or theme.tasklist_fg_secondary or theme.fg_secondary
    ...
    local floating_icon_active = args.floating_icon_active or theme.tasklist_floating_icon_active
    local floating_icon_inactive = args.floating_icon_inactive or theme.tasklist_floating_icon_inactive
    local maximised_icon_active = args.maximised_icon_active or theme.tasklist_maximised_icon_active
    local maximised_icon_inactive = args.maximised_icon_inactive or theme.tasklist_maximised_icon_inactive
    ...
    if client.floating.get(c) then
		if capi.client.focus == c then
			if c.maximized_horizontal and c.maximized_vertical then
				if maximised_icon_active then status_image = capi.image(maximised_icon_active) end
			else
				if floating_icon_active then status_image = capi.image(floating_icon_active) end
			end
		else
			if c.maximized_horizontal and c.maximized_vertical then
				if maximised_icon_inactive then status_image = capi.image(maximised_icon_inactive) end
			else
				if floating_icon_inactive then status_image = capi.image(floating_icon_inactive) end
			end
		end
    end
    ...
    elseif not client_in_selected_tags(c) and fg_secondary then
		text = text .. "<span color='"..util.color_strip_alpha(fg_secondary).."'>"..name.."</span>"
...
function client_in_selected_tags(c)
    for k, t in ipairs(capi.screen[c.screen]:tags()) do
        if t.selected then
            local ctags = c:tags()
